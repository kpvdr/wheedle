...
class GitHubPoller:
    ...
    def __init__(self):
        self.auth = (UID, GitHubPoller._get_github_token(os.path.join(DATA_DIR, TOKEN_FILE), True))
        # print('DEBUG: auth=%s' % (self.auth,))
    def start(self, sch=None):
        """ Start poller """
        try:
            repo_metadata = GitHubRepo(REPO_FULL_NAME, auth=self.auth)
            print('DEBUG: repo_metadata=%s' % repo_metadata.repo_metadata)
            if repo_metadata.is_disabled():
                raise DisabledRepoError(REPO_FULL_NAME)
            # commits = GitHubCommits(SOURCE_REPO_FULL_NAME,
            #                         auth=self.auth,
            #                         sha_file_name=os.path.join(DATA_DIR, 'last_sha'))
            # print('Num commits: %d' % commits.get_num_commits())
            # print('Last_commit: %s' % commits.get_last_commit()['sha'])
            artifacts = GitHubActionsArtifacts(REPO_FULL_NAME, auth=self.auth)
    ...
    @staticmethod
    def _get_github_token(token_file_name, throw_if_not_found):
        token = None
        if os.path.exists(token_file_name):
            with open(token_file_name) as token_file:
                token = token_file.read()[:-1] # strip trailing /n
        if token is None and throw_if_not_found:
